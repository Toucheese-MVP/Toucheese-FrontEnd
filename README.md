...
이 레포지토리는 터치즈MVP 프로젝트의 프론트엔드 코드를 포함하고 있습니다.  
...
- (noGnb), (withGnb) 그룹을 도입하여 **GNB 여부를 그룹별로 완전 분리**
- 불필요한 ClientGNBWrapper 제거 및 최적화
- GNB 유무에 따라 **페이지별 성능 및 렌더링 최적화**
- `/studio` → GNB 포함 / `/studio/[id]` → GNB 없음 구조 완성
- Next.js 14 App Router 기반으로 폴더 구조를 대규모 리팩토링
- 사용자 체감 속도 및 UI 안정성 향상
